...
function normalizeRegistrations(rows) {
  const lookup = new Map(
    (rows || []).map((r) => [
      r.month,
      { golfers: Number(r.golfers || 0), staff: Number(r.staff || 0) },
    ])
  );
...
export default function OperationalInsightsStatic() {
    ...
    const fetchRevenue = async () => {
      ...
      try {
        const response = await fetch(
          `${API_BASE_URL}/orders/course-admin/monthly-revenue?year=${selectedYear}`,
          {
            method: "GET",
            headers: {
        ...
        if (isMounted) {
          setRevenueRows(normalizeRevenue(data.monthlyRevenue));
        }
      } catch {
        if (isMounted) setRevenueRows(buildEmptyYearRevenue());
      }
  ...
  useEffect(() => {
    ...
    const fetchRegistrations = async () => {
      ...
      try {
        const response = await fetch(
          `${API_BASE_URL}/orders/course-admin/user-registrations-by-role?year=${selectedYear}`,
          {
            method: "GET",
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        if (isMounted) {
          setRegistrationRows(normalizeRegistrations(data.monthlyData));
        }
      } catch {
        if (isMounted) setRegistrationRows(buildEmptyYearRegistrations());
      }
  ...
  return (
    <section className="space-y-4">
      <h2 className="text-sm font-semibold text-gray-800">Operational Insights</h2>
      ...
      <ChartCard
        ...
        subtitle={`New staff user registrations for ${selectedYear}.`}
        headerRight={
          <label className="flex items-center gap-2 text-sm text-gray-600">
            <span>Year:</span>
            <select
      ...
      >
        <RegistrationByRoleChart data={registrationRows} showGolfers={false} />
      </ChartCard>
    </section>
  );
}
