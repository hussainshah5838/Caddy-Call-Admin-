...
import {
  getMenuImportCsvTemplate,
  isAllowedMenuImportFilename,
} from "./menuCsvTemplate";
import { postMenuCsvImport } from "../../utils/menuCsvImportApi";
...
const COLUMN_ROWS = [
  ...
  "Category (required)",
  "Menu Section (required only for Kitchen)",
  "Image URL (optional)",
...
function downloadTemplateCsv() {
  const csv = getMenuImportCsvTemplate();
 ...
 * @param {boolean} open
 * @param {() => void} onClose
 * @param {string} [scopedCourseName] - When set (Super Admin on menu/:id), import sends this course name. Omit for course admin.
 * @param {() => void} [onImported] - Called after a successful API response so the parent can refresh the list.
 * @param {(payload: { created: number, failed: number, errors: object[] }) => void} [onImportFinished] - Opens result UI in the parent.
 */
export default function UploadMenuCsvModal({
  open,
  onClose,
  scopedCourseName,
  onImported,
  ...
  const isSuperScoped = scopedCourseName !== undefined;
  const courseReady = !isSuperScoped || String(scopedCourseName || "").trim() !== "";
  ...
  const handleImport = async () => {
    ...
    setUploading(true);
    try {
      const data = await postMenuCsvImport(
        file,
        isSuperScoped ? String(scopedCourseName).trim() : undefined
      );
      onImported?.();
      onImportFinished?.({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="upload-menu-file-title"
    >
      <div
      ...
      >
        <div className="flex items-start justify-between gap-3 border-b border-gray-200 px-4 py-3 sm:px-5">
          <h2
            id="upload-menu-file-title"
            className="text-base font-semibold text-gray-900"
          >
            Upload menu (CSV or Excel)
          </h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="space-y-4 px-4 py-4 sm:px-5 sm:py-5">
          <p className="text-sm text-gray-600">
            Upload a CSV or Excel file (.xlsx, .xls) to import multiple menu
            items at once.
          </p>
